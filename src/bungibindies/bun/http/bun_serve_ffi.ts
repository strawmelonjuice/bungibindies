...
// Serve starter
export function Cserve(options: ServerOptions): GleamResult<Server, string> {
	const o = Bun.serve({
		// Static routes
		static: ((input): Record<`/${string}`, Response> => {
			...
			input.forEach((value: Response, key: string) => {
				if (!key.startsWith("/")) {
					return new GleamERROR("Static route must start with a /");
				}
				//@ts-expect-error
				output[key] = value;
			});
